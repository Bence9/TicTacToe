...
namespace TicTacToe
{
    public class GameState
    {
        ...
        private bool AreaSquareMarked((int , int)[] squares, Player player)
        {
        ...
        private bool DidMoveWin(int r, int c, out WinInfo winInfo)
        {
            ...
            if (AreaSquareMarked(row, CurrentPlayer))
            {
                winInfo = new WinInfo{Type = WinType.Row, Number = r};
                return true;
            }

            if (AreaSquareMarked(col, CurrentPlayer))
            {
                winInfo = new WinInfo { Type = WinType.Column, Number = c };
                return true;
            }

            if (AreaSquareMarked(MainDiag, CurrentPlayer))
            {
                winInfo = new WinInfo { Type = WinType.MainDiagonal };
                return true;
            }

            if (AreaSquareMarked(AntiDiag, CurrentPlayer))
            {
